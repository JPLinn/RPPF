...
class SampleSet(Dataset):
    def __init__(self, data_dir, usage):
        ...
        self.temp = np.load(os.path.join('../data/transfer_params/temp_features', f'{usage}_temp2.npy'))
...
class P2SRSet(Dataset):
    def __init__(self, st_ids: list, caps: list, pmodel_dir: str, usage='train'):
        data_dir = os.path.join('../..', 'data', 'single_station_experiment')
        for st_count, st_id in enumerate(st_ids):
            if st_count == 0:
                all_power = np.load(os.path.join(data_dir,
                                                 str(st_id).rjust(4, '0') + '_'+usage+'_power.npy')).reshape(-1, 1)
            else:
                tmp_power = np.load(os.path.join(data_dir,
                                                 str(st_id).rjust(4, '0') + '_'+usage+'_power.npy')).reshape(-1, 1)
                all_power = np.append(all_power, tmp_power, axis=1)
        self.label = (all_power * np.array(caps)).sum(axis=1) / sum(caps)
        self.input = np.load(os.path.join(pmodel_dir, usage+'_set_forecasts.npy'))
        self.length = self.label.shape[0]
...
class Evaluator(object):
    ...
    def report(self, usage='train'):
        ...
        return np.array([float('{:.4f}'.format(self.mqs)), float('{:.4f}'.format(self.mre)),
                         float('{:.4f}'.format(self.naps)), float('{:.4f}'.format(self.qcs))])
...
def plot_in_train(data, save_name, dir):
...
if __name__ == '__main__':
    data_dir = os.path.join('..', '..', 'data', 'single_station_experiment')
    data_set = SampleSet(data_dir)
    train_loader = DataLoader(data_set, batch_size=1024, shuffle=True, pin_memory=True, num_workers=4)