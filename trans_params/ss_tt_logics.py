...
class Fcst(object):
    def __init__(self, params: Params, model_dir, logger):
        ...
        self.struc = SCM(params).to(self.params.device)
    ...
    def __train__(self, data_loader: DataLoader):
        ...
        for (nwp_tensor, temp_tensor, label_tensor) in data_loader:
            ...
            loss = loss + self.struc.crps_qc(self.struc(nwp_tensor, temp_tensor), label_tensor, alpha=self.params.alpha)
            loss.backward()
            self.optimizer.step()
    ...
    def test_two(self, train_loader: DataLoader, test_loader: DataLoader, restore=True):
        ...
        evaluator = Evaluator(self.logger, self.params.q)
        with torch.no_grad():
            for (nwp_tensor, temp_tensor, label_tensor) in train_loader:
                nwp_tensor = nwp_tensor.to(self.params.device).float()
                label_tensor = label_tensor.to(self.params.device).float()
                temp_tensor = temp_tensor.to(self.params.device).float()
                nwp_tensor = nwp_tensor.permute(0, 2, 1)
                res = self.struc(nwp_tensor, temp_tensor)
                res[res < 0.] = 0.
                evaluator.batch_eval(res, label_tensor, usage='train')
            evaluator.sum_up(usage='train')
            evaluator.clear()
            for (nwp_tensor, temp_tensor, label_tensor) in test_loader:
                nwp_tensor = nwp_tensor.to(self.params.device).float()
                label_tensor = label_tensor.to(self.params.device).float()
                temp_tensor = temp_tensor.to(self.params.device).float()
                nwp_tensor = nwp_tensor.permute(0, 2, 1)
                res = self.struc(nwp_tensor, temp_tensor)
                res[res < 0.] = 0.
                evaluator.batch_eval(res, label_tensor, usage='test')
            evaluator.sum_up(usage='test')
...
class SingleOperator(object):
    def __init__(self, sta_id, thread_id=0):
        self.data_dir = os.path.join('..', 'data', 'transfer_params', 'single_station', str(sta_id).rjust(4, '0'))
        self.model_dir = os.path.join("ss_models", str(sta_id).rjust(4, '0'))
    ...
    def _set_logger(self):
        logger = logging.getLogger(f'SingleOperator_{self.sta_id}.log')
    ...
    def _set_params(self, thread_id):
        params = Params(os.path.join('ss_models', 'params.json'))
    ...
    def train(self):
        ...
        test_loader = DataLoader(test_set, batch_size=self.params.batch_size, shuffle=True,
                                 pin_memory=True, num_workers=4)
        self.logger.warning(f'***** Training and test of Station No.{self.sta_id} *****')
...
class Trials(object):
    def __init__(self, sta_id, id_thread=0, n_trials=10):
        self.data_dir = os.path.join('..', 'data', 'transfer_params', 'single_station', str(sta_id).rjust(4, '0'))
        self.model_dir = os.path.join("ss_models", str(sta_id).rjust(4, '0'))
        if not os.path.exists(self.model_dir):
            os.makedirs(self.model_dir)
        ...
        self.decided = None
        self.logger = self._set_logger()
        self.logger.warning(f'############ Trial of Station No.{sta_id} starts ############')
    ...
    def _set_logger(self):
        logger = logging.getLogger(f'Trials_{self.sta_id}.log')
        logger.setLevel(level=logging.INFO)
        formatter = logging.Formatter('%(asctime)s - %(levelname)s: %(message)s')
    ...
    def start(self):
        ...
        for k in range(self.n_trials):
            self.logger.warning(f'***** the {k + 1}_th time of Station No.{self.sta_id} *****')
            self._reinit_model()
            prefix = str(k+1)+'_'
    ...
    def decide(self):
        ...
        self.logger.warning(f'#### The {decided+1}-th model is selected for Station No.{self.sta_id} ####')
...
class Trials_for_hps(object):
    def __init__(self, params, n_trials=10):
        self.data_dir = os.path.join('..', 'data', 'transfer_params', 'single_station', str(params.sta_id).rjust(4, '0'))
        ...
        self.sta_id = params.sta_id
        self.decided = None
        self.logger = self._set_logger()
        self.logger.warning(f'############ Trial of Station No.{self.sta_id} starts ############')
        self.model = Fcst(self.params, self.model_dir, self.logger)
        self.res = np.zeros((self.n_trials, 2, 4))  # TODO: here is hard code.
...
if __name__ == '__main__':
    ...
    # res_save_dir = os.path.join('SStationResult', 'qs_20x')
    # if not os.path.exists(res_save_dir):
    #     os.makedirs(res_save_dir)
    station_ids = mcs.index.to_list()
    # mp_save_forecasts(station_ids, res_save_dir, n_threads=20)
    so = SingleOperator(station_ids[0])
    trial = Trials(station_ids[0], n_trials=20)
    trial.start()
