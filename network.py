...
# building blocks
# 1-order Chebyshev
class GConv1C(nn.Module):
    def __init__(self, params: Params):
        super(GConv1C, self).__init__()
        self.norm_lap = torch.tensor(np.load(params.lap_path), device=params.device).float()
        self.params = params
        self.trans10 = nn.Linear(in_features=self.params.gnn_input_dims, out_features=self.params.gnn_dims[0],
                                 bias=False)
        self.trans11 = nn.Linear(in_features=self.params.gnn_input_dims, out_features=self.params.gnn_dims[0],
                                 bias=False)
        self.trans20 = nn.Linear(in_features=self.params.gnn_dims[0], out_features=self.params.gnn_dims[1], bias=False)
        self.trans21 = nn.Linear(in_features=self.params.gnn_dims[0], out_features=self.params.gnn_dims[1], bias=False)
        self.act1 = nn.ReLU()
        self.act2 = nn.ReLU()
        # self.num_layers = len(params.dim_hidden)
        # for count, layer in enumerate(self.params.dim_hidden):
        #     if count == 0:
        #         self.trans1.append(
        #             nn.Linear(in_features=self.params.num_nwp, out_features=layer, bias=False).to(self.params.device))
        #         self.trans0.append(
        #             nn.Linear(in_features=self.params.num_nwp, out_features=layer, bias=False).to(self.params.device))
        #     else:
        #         self.trans1.append(
        #             nn.Linear(in_features=self.params.dim_hidden[count-1],
        #                       out_features=layer, bias=False).to(self.params.device))
        #         self.trans0.append(
        #             nn.Linear(
        #                 in_features=self.params.dim_hidden[count-1],
        #                 out_features=layer, bias=False).to(self.params.device))
        #     self.acts.append(nn.Softplus())
        self.output_size = self.norm_lap.size()[0] * self.params.gnn_dims[1]
    ...
    def forward(self, x: torch.Tensor):
        ...
        # output = self.act(self.trans0(x) + self.trans1(torch.matmul(self.norm_lap, x)))  # for 1-order Chebyshev
        y = self.act1(self.trans10(x) + self.trans11(torch.matmul(self.norm_lap, x)))
        y = self.act2(self.trans20(y) + self.trans21(torch.matmul(self.norm_lap, y)))
        return y.reshape((x.size()[0], -1))

    def reinit(self):
        lim10 = 1./np.sqrt(self.trans10.in_features)
        lim11 = 1./np.sqrt(self.trans11.in_features)
        lim20 = 1. / np.sqrt(self.trans20.in_features)
        lim21 = 1. / np.sqrt(self.trans21.in_features)
        nn.init.uniform_(self.trans10.weight, -lim10, lim10)
        nn.init.uniform_(self.trans11.weight, -lim11, lim11)
        nn.init.uniform_(self.trans20.weight, -lim20, lim20)
        nn.init.uniform_(self.trans21.weight, -lim21, lim21)


class GConv2C(nn.Module):
    def __init__(self, params: Params):
        super(GConv2C, self).__init__()
        self.norm_lap = torch.tensor(np.load(params.lap_path[0]), device=params.device).float()
        self.norm_lap2 = torch.tensor(np.load(params.lap_path[1]), device=params.device).float()
        self.params = params
        self.trans0 = nn.Linear(in_features=self.params.gnn_input_dims, out_features=self.params.gnn_dims[0],
                                bias=False)
        self.trans1 = nn.Linear(in_features=self.params.gnn_input_dims, out_features=self.params.gnn_dims[0],
                                bias=False)
        self.trans2 = nn.Linear(in_features=self.params.gnn_input_dims, out_features=self.params.gnn_dims[0],
                                bias=False)
        ...
        #                 out_features=layer, bias=False).to(self.params.device))
        #     self.acts.append(nn.Softplus())
        self.output_size = self.norm_lap.size()[0] * self.params.gnn_dims[0]
    ...
    def forward(self, x: torch.Tensor):
        ...
        #     output = \
        #         self.acts[layer](self.trans0[layer](output) + self.trans1[layer](torch.matmul(self.norm_lap, output)))
        output = self.act(self.trans0(x) + self.trans1(torch.matmul(self.norm_lap, x)) +
                          self.trans2(torch.matmul(self.norm_lap2, x)))
        return output.reshape((x.size()[0], -1))


# this is for 1-order approximate adjacent
class GConvAA(nn.Module):
    def __init__(self, params: Params):
        super(GConvAA, self).__init__()
        self.app_adj = torch.tensor(np.load(params.lap_path), device=params.device).float()
        self.params = params
        self.trans = nn.Linear(in_features=self.params.gnn_input_dims, out_features=self.params.gnn_dims[0], bias=True)
        self.act = nn.Softplus()
        self.output_size = self.app_adj.size()[0] * self.params.gnn_dims[0]
    ...
    def forward(self, x: torch.Tensor):
        ...
        #     output = \
        #         self.acts[layer](self.trans0[layer](output) + self.trans1[layer](torch.matmul(self.norm_lap, output)))
        output = self.act(self.trans(torch.matmul(self.app_adj, x)))  # for 1-order Chebyshev
        return output.reshape((x.size()[0], -1))


# this is for global experiment
class Conv1DT1(nn.Module):
    def __init__(self, params: Params):
        super(Conv1DT1, self).__init__()
        self.params = params
        self.conv1 = nn.Conv1d(in_channels=self.params.num_nwp, out_channels=self.params.cnn_hidden_dims[0],
                               kernel_size=80, padding=0)
        self.conv2 = nn.Conv1d(in_channels=self.params.cnn_hidden_dims[0], out_channels=self.params.cnn_hidden_dims[1],
                               kernel_size=80, padding=0)
        self.conv3 = nn.Conv1d(in_channels=self.params.cnn_hidden_dims[1], out_channels=self.params.cnn_hidden_dims[2],
                               kernel_size=80, padding=0)
        # TODO how to
        self.output_size = self.params.cnn_hidden_dims[2] * 18
        self.act = nn.Softplus()

    def forward(self, x: torch.Tensor):
        # x is (batch_size, num_nodes, num_features)
        return self.act(self.conv3(self.conv2(self.conv1(x[:, :, :].permute(0, 2, 1)))).reshape((x.size()[0], -1)))
        # flatten the output, so its size is (batch_size, output_features)


# this is for global experiment
class Conv1DT0(nn.Module):
    def __init__(self, params: Params):
        super(Conv1DT0, self).__init__()
        self.params = params
        self.conv = nn.Conv1d(in_channels=self.params.num_nwp, out_channels=self.params.cnn_dims[0],
                              kernel_size=self.params.cnn_nwp_nodes_num)

        self.output_size = self.params.cnn_dims[0]
        self.act = nn.Softplus()
...
class LocalConv1D(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        return self.act(self.conv(x.permute(0, 2, 1))).reshape((x.size()[0], self.params.num_stats, -1))
...
# bonding
# using conv1d on every stations and then send them to mlp
class CMap(nn.Module):
    def __init__(self, params: Params):
        super(CMap, self).__init__()
        self.params = params
        self.CV = Conv1DT0(self.params)
        self.OL = QuantileOutputLayer(input_size=self.CV.output_size+self.params.num_temp, q=self.params.q,
                                      bq=self.params.weights)

    def forward(self, x1: torch.Tensor, x2: torch.Tensor):
        # x1 is (batch_size, num_nodes, num_nwp_features), x2 is (batch_size, num_temporal_features)
        return self.OL(torch.cat((self.CV(x1), x2), dim=1))
    ...
    def crps_loss(self, pred: torch.Tensor, label: torch.Tensor):
        ...
        return crps / label.numel()


# this is for sing station experiment
class GCMap(nn.Module):
    def __init__(self, params: Params):
        super(GCMap, self).__init__()
        self.params = params
        self.GC = GConvAA(self.params)
        self.OL = QuantileOutputLayer(input_size=self.GC.output_size+self.params.num_temp, q=self.params.q,
                                      bq=self.params.weights)

    def forward(self, x1: torch.Tensor, x2: torch.Tensor):
        # x1 is (batch_size, num_nodes, num_nwp_features), x2 is (batch_size, num_temporal_features)
        return self.OL(torch.cat((self.GC(x1), x2), dim=1))
...
# TODO: unfinished
class GMMOutputLayer(nn.Module):
    def __init__(self, input_size, num_kernels=5):
        super(GMMOutputLayer, self).__init__()
        self.input_size = input_size
        self.num_kernels = num_kernels

        self.weight_front = nn.Linear(in_features=input_size, out_features=self.num_kernels)
        self.weight_rear = nn.Softmax(dim=-1)

        self.mu = nn.Linear(in_features=input_size, out_features=self.num_kernels)

        self.sigma_front = nn.Linear(in_features=input_size, out_features=self.num_kernels)
        self.sigma_rear = nn.Softplus()

    def forward(self, x):
        return self.sp(self.aff(x)).cumsum(dim=1)
